...
void print_dir(char *dirname, int fd) {
...
int main(int argc, char *argv[]) {
    ...
    if (outfile == NULL)
        ...
        if ((fd = open(outfile, O_WRONLY | O_CREAT | O_TRUNC, 0644)) < 0) // to nie tworzy pliku
            ERR("open");

    for (int i=0; i<dircount; i++)
        print_dir(dirs[i], fd);

    if (outfile != NULL && close(fd) == -1)
        ERR("close");
